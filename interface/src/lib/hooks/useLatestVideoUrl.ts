 ...
 */
export function useLatestVideoUrl(clip: FullClip | null | undefined, opts?: { skewSeconds?: number }) {
    const skewMs = (opts?.skewSeconds ?? 30) * 1000;

    const { needsRefresh, guildId, clipId } = useMemo(() => {
        if (!clip) return { needsRefresh: false, guildId: undefined as string | undefined, clipId: undefined as string | undefined };
        const expiresAt = new Date(clip.clip.expires_at).getTime();
        const now = Date.now();
        const needs = isFinite(expiresAt) ? expiresAt - now <= skewMs : true;
        return { needsRefresh: needs, guildId: clip.clip.guild_id, clipId: clip.clip.id };
    }, [clip, skewMs]);

    // Query server for refreshed clip if needed; server will refresh CDN URL if expired.
    const { data: refreshed, isLoading } = useClip(
        guildId ?? "",
      	clipId ?? ""
    );
    ...
    const effectiveUrl = useMemo(() => {
        ...
        if (needsRefresh && refreshed?.clip?.cdn_url) return refreshed.clip.cdn_url;
