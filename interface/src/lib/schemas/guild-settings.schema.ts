import { z } from "zod";
import { isValidTimezone, normalizeTimezone } from "../utils/timezone-helpers";

// Validation helpers
const cronRegex =
    /^(@(annually|yearly|monthly|weekly|daily|hourly|continuous))|(\S+\s+\S+\s+\S+\s+\S+\s+\S+)$/;
    ...
    .object({
        ...
        tz: z
            ...
            .refine(isValidTimezone, {
                message:
                    "Invalid timezone. Use abbreviations (PST, EST, UTC) or IANA format (America/Los_Angeles)",
            })
            .optional(),
        schedule_cron: z
            .string()
            .regex(
                cronRegex,
                "Invalid cron expression. Use standard cron or @hourly, @daily, etc."
            )
            .optional(),
    })
 ...
 * of the `guild_settings` table.
 */
export const DefaultChannelSettingsSchema = z
    .object({
        // =========================
        // BASIC
        // =========================
 ...
 */
export const UpdateGuildSettingsPayloadSchema = z
    .object({
        guild_id: z.string().min(1),
        settings: GuildSettingsSchema.partial().optional(),
        default_channel_settings:
            DefaultChannelSettingsSchema.partial().optional(),
    })
    .strict();

// Type exports for TypeScript
export type GuildSettings = z.infer<typeof GuildSettingsSchema>;
export type DefaultChannelSettings = z.infer<
    typeof DefaultChannelSettingsSchema
>;
export type UpdateGuildSettingsPayload = z.infer<
    typeof UpdateGuildSettingsPayloadSchema
>;
