...
export default function ChannelsList({ channels }: ChannelsListProps) {
	return (
		<div className="mt-6">
			<h2 className="text-xl font-semibold mb-3">Channels ({channels.length})</h2>
			{channels.length === 0 ? (
				<p className="text-muted-foreground">No channels found for this guild.</p>
			) : (
		...
		>
			<div className="flex items-center justify-between">
				...
				<div className="flex flex-col items-end gap-1">
					...
					>
						{channel.message_scan_enabled ? "Scan: ON" : "Scan: OFF"}
					</span>
					{channel.last_channel_sync_at && (
						<span className="text-xs text-muted-foreground">
							Last sync: {new Date(channel.last_channel_sync_at).toLocaleDateString()}
						</span>
					)}
				</div>
			</div>
